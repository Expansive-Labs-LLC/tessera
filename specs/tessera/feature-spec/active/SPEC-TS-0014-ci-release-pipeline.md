...
| FR-021 | The `@semantic-release/git` plugin SHALL commit back `CHANGELOG.md` only, with `[skip ci]` in the message. It SHALL NOT commit back `tessera/__init__.py` or `blender_manifest.toml`: those carry a `0.0.0` placeholder in source that `build_addon.sh` patches at build time. Committing a real version into source conflicts with every open branch on every release, and drifts whenever a local build is run. |
...
| FR-027 | The build script SHALL patch the `bl_info["version"]` tuple in `tessera/__init__.py` by replacing the line matching `"version": (<digits>, <digits>, <digits>),` with `"version": (<major>, <minor>, <patch>),` using `sed`. The replacement SHALL use a regex that matches the existing tuple format exactly: `"version": ([0-9]*, [0-9]*, [0-9]*),`.. The committed value SHALL be the placeholder `(0, 0, 0)`, and the script SHALL restore both version placeholders on exit (including on failure) so that a build never leaves a real version in the working tree. |
...
| TS-009 | `.releaserc.yml` `@semantic-release/git` assets list contains `CHANGELOG.md` and excludes `tessera/__init__.py` and `blender_manifest.toml` | Manual | FR-021 | Must Pass |
