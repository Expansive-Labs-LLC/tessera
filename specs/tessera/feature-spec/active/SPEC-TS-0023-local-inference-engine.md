...
|---|---|---|
| **Distribution** | A signed native installer carrying its own pinned CPython, virtual environment and prebuilt wheels — **Linux x64 in v1** (FR-030) | A `pipx` bootstrap presumes a Python the user does not have and a toolchain they cannot be asked to install; a container presumes Docker. Only a native installer meets ADR-0001 D2 for an audience of artists. Windows is deferred to SPEC-TS-0024 on a code-signing identity; macOS waits on non-CUDA support |
...
| ID | Requirement |
|----|-------------|
| FR-030 | The engine SHALL ship as a signed platform-native installer carrying its own pinned CPython interpreter, virtual environment and prebuilt wheels, so installation requires no pre-existing Python, no compiler and no `pip` invocation by the user. **v1 SHALL supply a Linux x64 installer only.** Windows x64 is deferred to SPEC-TS-0024, not cancelled: it needs a code-signing identity that is a procurement lead time rather than engineering work, and shipping it unsigned would trip SmartScreen and violate ADR-0001 D2 more thoroughly than not shipping it. macOS is excluded for longer, because v1 is CUDA-only (FR-024, PRD-001 D7/NG8). PyTorch is consumed as a published `cu128` wheel and needs no build. TRELLIS's compiled extensions have no such wheel for current architectures and SHALL be built in CI against a CUDA toolkit that supports every target compute capability — the build matrix is per GPU architecture, not only per platform (ADR-0001 premise verification). |
| FR-031 | The installer SHALL write an installation marker recording `engine_version`, the protocol versions the engine speaks, and the absolute path of the engine executable, at `${XDG_DATA_HOME:-~/.local/share}/tessera/engine/install.json`. A readable marker is the definition of *installed* (EC-001); the add-on SHALL NOT probe the port to answer that question. The add-on's discovery already resolves the Windows location too, so the deferred Windows installer (FR-030) is a packaging change rather than a client change. |
...
| FR-041 | The add-on SHALL fetch the engine installer through the existing download manager (SPEC-TS-0002), against a pinned release URL and a manifest digest, and SHALL verify that digest before handing the file to the operating system to run. It SHALL NOT open its own network path, so SPEC-TS-0002 CON-001's single network module and the integrity guarantees that go with it continue to hold for the engine as they do for weights. A digest mismatch SHALL delete the download and report it, exactly as FR-007 of SPEC-TS-0002 does. The artifact is multiple gigabytes (NFR-012), so the resume, progress reporting and retry that module already provides are load-bearing here rather than incidental — a plain fetch of a file this size fails often enough to be unusable. |
| FR-042 | The Linux installer SHALL be signed with a detached OpenPGP signature over the release artifact, published alongside it with the signing key fingerprint, and the release notes SHALL carry the SHA256 the add-on verifies against (FR-041). Signature verification is the user's to perform and the distribution's to make possible; the add-on's own gate is the pinned digest, which does not depend on the user having imported a key. |
...
| NFR-008 | Engine install, unaided completion | Supervised first-time users | ≥ 90%, n ≥ 10 | Usability testing on Linux x64, the only v1 platform (FR-030) |
...
| NFR-012 | Engine artifact size | Compressed release artifact | ≤ 2 GiB, enforced by the build | A single release asset cannot exceed 2 GiB and the artifact is not moving off that host, so this is a hard ceiling rather than a target. Untrimmed it was 3.7 GiB gzipped; trimming build-time content and Triton and switching to `xz -9` gives 2.12 GiB, and stubbing the linked-but-unreachable NCCL brings it to **1.97 GiB — 29 MB of headroom**. TRELLIS is not in that figure. The build fails rather than emitting an oversized artifact |
| NFR-013 | Engine install download, resumed | A download interrupted at any point resumes rather than restarting | 100% | The artifact is measured in gigabytes, so a fetch that cannot resume is a fetch that frequently never completes (FR-041) |
...
- ❌ Windows engine installer — deferred to SPEC-TS-0024 pending a code-signing identity (FR-030). The add-on's discovery, client and status model are already platform-neutral, so this is a packaging gap, not a design one
- ❌ macOS and Apple Silicon engine builds — v1 is CUDA-only (FR-024), so there is no device path to install for
...
| Engine installer artifacts | Yes | Linux x64 only in v1 (FR-030); Windows deferred to SPEC-TS-0024 |
...
| TS-038 | Linux x64 installer completes with no pre-existing Python and no compiler | Manual | FR-030, NFR-008 | Must Pass |
| TS-040 | Release artifact carries a detached OpenPGP signature that verifies against the published fingerprint | Script | FR-042 | Must Pass |
| TS-041 | Installer writes the installation marker the add-on discovers it by | Integration | FR-031 | Must Pass |
| TS-042 | Release artifact stays under the NFR-012 size budget | Script | NFR-012 | Must Pass |
| TS-043 | Interrupted engine-installer download resumes from where it stopped | Integration | NFR-013, FR-041 | Must Pass |
...
| 1.7 | 2026-09-24 | Derek | Rewrote NFR-012 around the constraint that actually binds. A release asset cannot exceed 2 GiB and the decision is to trim rather than host elsewhere, so the budget is now a ceiling the build enforces rather than a target it reports. Measured: 3.7 GiB gzipped untrimmed, 2.12 GiB after removing build-time content and Triton and moving to `xz -9`, and 1.97 GiB once NCCL is stubbed — 29 MB of headroom, with TRELLIS still to come. Recorded that `nvprune` cannot prune linked shared libraries and that cuSPARSELt is called during torch initialisation, so neither is available as a further lever. |
| 1.6 | 2026-09-23 | Derek | Added NFR-012 and NFR-013 after building a real artifact: a Linux x64 build carrying CPython, PyTorch `cu128` and the CUDA runtime libraries measures 3.7 GiB before TRELLIS is in it at all. Nothing in the spec had budgeted for that, and it changes what FR-041 is relying on — the resume, progress and retry the existing download manager provides are load-bearing at this size rather than incidental, since a plain fetch of a multi-gigabyte file fails often enough to be unusable. |
| 1.5 | 2026-09-23 | Derek | Narrowed v1 to a Linux x64 engine installer and deferred Windows to SPEC-TS-0024. Windows needs a code-signing identity, which is procurement lead time rather than engineering work, and an unsigned Windows installer trips SmartScreen — worse for ADR-0001 D2 than not shipping one. The add-on's discovery, client and status model already resolve both platforms, so what is deferred is packaging. Added FR-042 for detached OpenPGP signing of the Linux artifact, with the add-on's own integrity gate remaining the pinned digest of FR-041, which does not depend on the user having imported a key. |
