...
| NFR-012 | Engine artifact size | Compressed release tarball | < 6 GiB | Measured at **3.7 GiB** for a Linux x64 build carrying CPython, PyTorch `cu128` and the CUDA runtime libraries — before TRELLIS and its extensions. The budget leaves room for those; exceeding it means the artifact needs splitting, not a larger number |
